...
var price = () => {
  ...
  document.getElementById("total").value =
    "Rs. " + (adults * (date1 - date2) * 1000) / (1000 * 60 * 60 * 24);
};
...
var hotelDetail = async () => {
  ...
  const response = await fetch(
    `https://travel-advisor.p.rapidapi.com/hotels/get-details?location_id=${locationId}&adults=1&lang=en_US&currency=USD&nights=2`,
    options
  );
...
const fomrHandler = (e) => {
  ...
  window.location.assign(
    `payment.html?location_id=${locationId}&adult=${adult}&name=${name}&fromDate=${fromDate}&toDate=${toDate}&total=${total}`
  );
};
