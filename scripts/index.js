...
var onSearch = async () => {
  ...
  if (input.length >= 3) {
    ...
    const response = await fetch(
      `https://travel-advisor.p.rapidapi.com/locations/v2/auto-complete?query=${input.toLowerCase()}&lang=en_US&units=km`,
      options
    );
    ...
    for (var i = 0; i < result.length; i++) {
      if (
        result[i].__typename == "Typeahead_LocationItem" &&
        result[i].detailsV2.placeType == "CITY"
      ) {
